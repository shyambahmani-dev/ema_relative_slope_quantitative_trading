...
class run_strat(object):
    ...
    def run(self, slowSignalBuy, slowSignalSell, buyAbove, sellBelow):
        ...
        for ind in self.data1.index:
            ...
            if(not pd.isna(self.data1EMASlopeRel["%s" %(max(slowSignalBuy, slowSignalSell))].loc[ind])):
                ...
                if( self.data1EMASlopeRel["%s" %(slowSignalBuy)].loc[ind] > buyAbove):
                ...
                elif( self.data1EMASlopeRel["%s" %(slowSignalSell)].loc[ind] < sellBelow):
...
for slowSignalBuy in emaList:

    for slowSignalSell in emaList:

        for fastSignalBuy in emaList:
            
            for fastSignalSell in emaList:
                    ...
                    slopeMinBuy = (data1EMASlopeRel["%s" %(slowSignalBuy)].mean() - 2*data1EMASlopeRel["%s" %(slowSignalBuy)].std())
                    slopeMaxBuy = (data1EMASlopeRel["%s" %(slowSignalBuy)].mean() + 2*data1EMASlopeRel["%s" %(slowSignalBuy)].std())
                    slopeListBuy = np.arange( slopeMinBuy, slopeMaxBuy, ( (slopeMaxBuy - slopeMinBuy)/10 ))

                    slopeMinSell = (data1EMASlopeRel["%s" %(slowSignalSell)].mean() - 2*data1EMASlopeRel["%s" %(slowSignalSell)].std)
                    slopeMaxSell = (data1EMASlopeRel["%s" %(slowSignalSell)].mean() + 2*data1EMASlopeRel["%s" %(slowSignalSell)].std)
        ...
        for buyAbove in slopeListBuy:
            ...
            for sellBelow in slopeListSell:
                ...
                if( slowSignalBuy <= slowSignalSell):

                    print("Running strat for %s-%s-%s-%s" %(slowSignalBuy, slowSignalSell, buyAbove, sellBelow))
                    
                    stratRes = strat_object.run(slowSignalBuy, slowSignalSell, buyAbove, sellBelow)
                                
                    CAGRExcessRes.append( [stratRes.CAGRExcess , "%s-%s-%s-%s" %(slowSignalBuy, slowSignalSell, buyAbove, sellBelow)] )
                    AUCRatioRes.append( [stratRes.AUCRatio , "%s-%s-%s-%s" %(slowSignalBuy, slowSignalSell, buyAbove, sellBelow)] )
                    maxUpRes.append( [stratRes.maxUp , "%s-%s-%s-%s" %(slowSignalBuy, slowSignalSell, buyAbove, sellBelow)] )
                    maxDownRes.append( [stratRes.maxDown , "%s-%s-%s-%s" %(slowSignalBuy, slowSignalSell, buyAbove, sellBelow)] )
                    averageUpRes.append( [stratRes.averageUp , "%s-%s-%s-%s" %(slowSignalBuy, slowSignalSell, buyAbove, sellBelow)] )
