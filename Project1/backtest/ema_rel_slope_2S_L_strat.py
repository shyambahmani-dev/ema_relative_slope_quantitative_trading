...
strat_name = 'EMA Relative Slope Strategy - 2 Signals'
...
for ind in data1.index:
    ...
    if(not pd.isna(data1EMA["%s" %(max(slowSignalBuy, slowSignalSell))].loc[ind])):
        ...
        if( data1EMASlopeRel["%s" %(slowSignalBuy)].loc[ind] > slowBuyAbove and data1EMASlopeRel["%s" %(fastSignalBuy)].loc[ind] > fastBuyAbove):
        ...
        elif( data1EMASlopeRel["%s" %(slowSignalSell)].loc[ind] < slowSellBelow and data1EMASlopeRel["%s" %(fastSignalSell)].loc[ind] < fastSellBelow):
